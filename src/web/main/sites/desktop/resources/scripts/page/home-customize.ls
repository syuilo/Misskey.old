			...
			$widget-lapper.mousedown (e) ->
				function end-move(x, y)
					...
					if not moved
						...
						if (($ '#left-contents *').length == 0) and (x > left-area-position.left) and (x < left-area-position.left + left-area-width) and (y > left-area-position.top) and (y < left-area-position.top + left-area-height)
						...
						if (($ '#right-contents *').length == 0) and (x > right-area-position.right) and (x < right-area-position.right + right-area-width) and (y > right-area-position.top) and (y < right-area-position.top + right-area-height)
