...
module.exports = (req, res, options) ->
				...
				Promise.all (followers |> map (follower) ->
					...
					User.find-by-id follower.follower-id, (, follower-user) ->
						...
						if me?
							...
							follower-user.is-following = null
							follower-user.is-follow-me = null
							resolve follower-user)
				.then (followers) ->
					resolve followers
	...
	function get-you-dont-know(me-following-ids)
		...
		if me? and me-following-ids?
			UserFollowing
				.find {followee-id: user.id} `$and` {follower-id: {$nin: me-following-ids}}
				.sort {created-at: \desc}
				.limit 100users
				.exec (, followers) ->
					Promise.all (followers |> map (follower) ->
	...
	function get-you-dont-know-count(me-following-ids)
		resolve, reject <- new Promise!
		if me? and me-following-ids?
			UserFollowing.count {followee-id: user.id} `$and` {follower-id: {$nin: me-following-ids}} (err, c) ->
				resolve c
		else
			resolve null
			...
			get-all! .then (followers) ->
				get-you-know-count me-following-ids .then (followers-you-know-count) ->
					get-you-know me-following-ids .then (followers-you-know) ->
						get-you-dont-know-count me-following-ids .then (followers-you-dont-know-count) ->
							get-you-dont-know me-following-ids .then (followers-you-dont-know) ->
								res.display req, res, \user-followers {
									followers-count
									followers-you-know-count
									followers-you-dont-know-count
									followers
									followers-you-know
									followers-you-dont-know
									user
									page: \followers
								}
